...
To deploy the backend, we build a Docker image that takes in secret parameters as container environment variables. This image only runs processes that are required for the backend service, similar to a Unikernel, and so the attack surface is much smaller. This image is pushed to the public Docker Hub under `monarchsofcoding/chitchat:release-\emph{version}' where \emph{version} is the git SHA of the build.
